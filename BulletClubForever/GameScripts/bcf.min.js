...
﻿LevelMap=function(x,y){this.map=[];this.x=x;this.y=y;}
LevelMap.prototype={init:function(){for(var i=0;i<this.x;i++){this.map[i]=[];for(var j=0;j<this.y;j++){this.map[i][j]=".";}}
this.initPlatforms();this.overrideFirstPlatform();},initPlatforms:function(){for(var i=0;i<this.x;i++){this.makePieceOfMap(i);}},overrideFirstPlatform:function(){this.map[0][this.y-1]="G";},addNewPieceOfMap:function(){this.map[this.x]=[];this.makePieceOfMap(this.x);this.x++;},makePieceOfMap:function(xAxisPos){var hasGround=!createRandomBool(9);var groundPiece="."
...
function createRandom(n){return Math.floor((Math.random()*n)+1);}
...
Player.prototype.preload=function(){this.game.load.image('player','Sprites/player.png');this.game.load.image('playerFeet','Sprites/player-feet.png');}
Player.prototype.create=function(){this.feet=this.game.add.sprite(0,440,'playerFeet');this.playerSprite=this.game.add.sprite(-6,-22,'player');this.feet.addChild(this.playerSprite);this.game.physics.arcade.enable(this.feet);this.feet.body.collideWorldBounds=true;this.feet.body.gravity.y=this.GRAVITY;this.feet.body.maxVelocity.setTo(this.MAXSPEED,this.JUMPSPEED);this.keyboard=this.game.input.keyboard;this.cursorKeys=this.keyboard.createCursorKeys();}
Player.prototype.update=function(platforms){var self=this;platforms.group.forEach(function(child){if(self.feet.body.y<child.body.y+self.feet.body.height){self.game.physics.arcade.collide(self.feet,child);}});if(this.cursorKeys.right.isDown){this.feet.body.acceleration.x=this.ACCELLERATION;}
